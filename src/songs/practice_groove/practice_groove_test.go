...
func songLower() {
	...
	melodyLower([]string{"I", "iii", "IV", "ii", "VI"}, 0, 2, nil)
...
func metronome(measures int, tempo string, beats BeatEntries) []string {
	if beats.BeatsPerMeasure == 0 {
		beats.BeatsPerMeasure = 4
	}
	if beats.CountBeat == 0 {
		beats.CountBeat = 4
	}
	if beats.Downbeat == "" {
		beats.Downbeat = fmt.Sprintf("note:20,time:P+1/%d", beats.CountBeat)
	}
	if beats.Offbeat == "" {
		beats.Offbeat = fmt.Sprintf("note:-8,time:P+1/%d", beats.CountBeat)
	}

	notes := []string{}
	notes = append(notes, header("metronome", "C4", "major", tempo)...)
	for i := 0; i < measures; i++ {
		if i%beats.BeatsPerMeasure == 0 {
			notes = append(notes, beats.Downbeat)
		} else {
			notes = append(notes, beats.Offbeat)
		}
	}
	return notes
}
func funkyTown() []string {
	...
	outputChords = append(outputChords, "time:P+1/8")
	for i := range chordsOfNote {
		outputChords = append(outputChords, fmt.Sprintf("chord:%s,time:P+%s,dur:%s", chordsOfNote[i*2%len(chordsOfNote)], beatPattern[i], beatPattern[i]))
	}
	outputChords = append(outputChords, "time:P+1/8")
...
func NotesBeats(notes []string, beat []string) []string {
	outNotes := []string{}
	for i, note := range notes {
		outNotes = append(outNotes, fmt.Sprintf("%s,time:%s", note, beat[i%(len(beat)-1)]))
	}
	return outNotes

}
func outMajor() string {
	return `   chord:I,dur:1/4,split:1,chord_type:major,key_type:major,key_note:C4,time:P+1/4
				chord:ii,time:P+1/4
